...
class Context:
  ...
  Jaref: wp.array(dtype=wp.float32, ndim=2)
  ...
  jv: wp.array(dtype=wp.float32, ndim=2)
  quad: wp.array(dtype=wp.float32, ndim=3)
...
def _context(m: types.Model, d: types.Data) -> Context:
  ...
  ctx.Jaref = wp.empty(shape=(d.nworld, d.nefc), dtype=wp.float32)
  ctx.efc_force = wp.empty(shape=(d.nworld, d.nefc), dtype=wp.float32)
  ...
  ctx.active = wp.empty(shape=(d.nworld, d.nefc), dtype=wp.int32)
  ctx.gtol = wp.empty(shape=(d.nworld), dtype=wp.float32)
  ctx.mv = wp.empty(shape=(d.nworld, m.nv), dtype=wp.float32)
  ctx.jv = wp.empty(shape=(d.nworld, d.nefc), dtype=wp.float32)
  ctx.quad = wp.empty(shape=(d.nworld, d.nefc, 3), dtype=wp.float32)
  ...
  ctx.cost_warmstart = wp.empty(shape=(d.nworld), dtype=wp.float32)
...
def _create_context(ctx: Context, m: types.Model, d: types.Data, grad: bool = True):
  ...
  def _jaref(ctx: Context, m: types.Model, d: types.Data):
    ...
    wp.atomic_add(ctx.Jaref[worldid], efcid, d.efc_J[worldid, efcid, dofid] * d.qacc[worldid, dofid] - d.efc_aref[worldid, efcid] / float(m.nv))
  wp.launch(_jaref, dim=(d.nworld, d.nefc, m.nv), inputs=[ctx, m, d])
  ...
  if wp.static(grad):
    ...
    # search = -Mgrad
    @wp.kernel
    def _search(Mgrad: wp.array(ndim=2, dtype=wp.float32), search: wp.array(ndim=2, dtype=wp.float32)):
...
def _create_lspoint(ls_pnt: LSPoint, m: types.Model, d: types.Data, ctx: Context):
  ...
  @wp.kernel
  def _quad(ctx: Context):
    worldid, efcid = wp.tid()
    x = ctx.Jaref[worldid, efcid] + ctx.alpha[worldid] * ctx.jv[worldid, efcid]
    # TODO(team): active and conditionally active constraints
    active = float(x < 0.0)
    wp.atomic_add(ctx.quad_total[worldid], 0, ctx.quad[worldid, efcid, 0] * active)
    wp.atomic_add(ctx.quad_total[worldid], 1, ctx.quad[worldid, efcid, 1] * active)
    wp.atomic_add(ctx.quad_total[worldid], 2, ctx.quad[worldid, efcid, 2] * active)
  wp.launch(_quad, dim=(d.nworld, d.nefc), inputs=[ctx])
...
def _update_constraint(m: types.Model, d: types.Data, ctx: Context):
  ...
  def _active(ctx: Context):
    worldid, efcid = wp.tid()
    # TODO(team): active and conditionally active constraints
    ctx.active[worldid, efcid] = int(ctx.Jaref[worldid, efcid] < 0.0)
  wp.launch(_active, dim=(d.nworld, d.nefc), inputs=[ctx])

  @wp.kernel
  def _efc_force(ctx: Context, d: types.Data):
    worldid, efcid = wp.tid()
    ctx.efc_force[worldid, efcid] = -1.0 * d.efc_D[worldid, efcid] * ctx.Jaref[worldid, efcid] * float(ctx.active[worldid, efcid])
  wp.launch(_efc_force, dim=(d.nworld, d.nefc), inputs=[ctx, d])
  ...
  def _cost(ctx: Context, d: types.Data):
    worldid, efcid = wp.tid()
    Jaref = ctx.Jaref[worldid, efcid]
    wp.atomic_add(ctx.cost, worldid, 0.5 * d.efc_D[worldid, efcid] * Jaref * Jaref * float(ctx.active[worldid, efcid]))
  wp.launch(_cost, dim=(d.nworld, d.nefc), inputs=[ctx, d])
  ...
  def _qfrc_constraint(ctx: Context, d: types.Data):
    ...
    wp.atomic_add(ctx.qfrc_constraint[worldid], dofid, d.efc_J[worldid, efcid, dofid] * ctx.efc_force[worldid, efcid])
  wp.launch(_qfrc_constraint, dim=(d.nworld, m.nv, d.nefc), inputs=[ctx, d])
  ...
  def _gauss(ctx: Context, d: types.Data):
    worldid, dofid = wp.tid()
    wp.atomic_add(ctx.gauss, worldid, 0.5 * (ctx.Ma[worldid, dofid] - d.qfrc_smooth[worldid, dofid]) * (ctx.qacc[worldid, dofid] - d.qacc_smooth[worldid, dofid]))
...
def _update_gradient(m: types.Model, d: types.Data, ctx: Context):
  ...
  def _grad(ctx: Context, d: types.Data):
    worldid, dofid = wp.tid()
    ctx.grad[worldid, dofid] = ctx.Ma[worldid, dofid] - d.qfrc_smooth[worldid, dofid] - ctx.qfrc_constraint[worldid, dofid]
  wp.launch(_grad, dim=(d.nworld, m.nv), inputs=[ctx, d])

  if wp.static(m.opt.solver) == 1:  # CG
    smooth.solve_m(m, d, ctx.grad, ctx.Mgrad)
  elif wp.static(m.opt.solver) == 2:  # Newton
    # TODO(team): sparse version
    # h = qM + (efc_J.T * efc_D * active) @ efc_J
    wp.copy(ctx.h, d.qM)

    @wp.kernel
    def _JTDAJ(J: wp.array(ndim=3, dtype=wp.float32), D: wp.array(ndim=2, dtype=wp.float32), A: wp.array(ndim=2, dtype=wp.int32), output: wp.array(ndim=3, dtype=wp.float32)):
      worldid, dofi, dofj, efcid = wp.tid()
      wp.atomic_add(output[worldid, dofi], dofj, J[worldid, dofi, efcid] * D[worldid, efcid] * float(A[worldid, efcid]) * J[worldid, dofj, efcid])
    wp.launch(_JTDAJ, dim=(d.nworld, m.nv, m.nv, d.nefc), inputs=[d.efc_J, d.efc_D, ctx.active, ctx.h])
...
def _linesearch(m: types.Model, d: types.Data, ctx: Context):
  ...
  def _jv(ctx: Context, d: types.Data):
    worldid, efcid, dofid = wp.tid()
    wp.atomic_add(ctx.jv[worldid], efcid, d.efc_J[worldid, efcid, dofid] * ctx.search[worldid, dofid])
  wp.launch(_jv, dim=(d.nworld, d.nefc, m.nv), inputs=[ctx, d])
  ...
  def _quad_gauss12(ctx: Context, d: types.Data):
    ...
    wp.atomic_add(ctx.quad_gauss[worldid], 1, search * (ctx.Ma[worldid, dofid] - d.qfrc_smooth[worldid, dofid]))
  ...
  def _quad(ctx: Context, d: types.Data):
    worldid, efcid = wp.tid()
    Jaref = ctx.Jaref[worldid, efcid]
    jv = ctx.jv[worldid, efcid]
    efc_D = d.efc_D[worldid, efcid]
    ctx.quad[worldid, efcid, 0] = 0.5 * Jaref * Jaref * efc_D
    ctx.quad[worldid, efcid, 1] = jv * Jaref * efc_D
    ctx.quad[worldid, efcid, 2] = 0.5 * jv * jv * efc_D
  wp.launch(_quad, dim=(d.nworld, d.nefc), inputs=[ctx, d])
  ...
  @wp.kernel
  def _lo_alpha(lo: LSPoint, p0: LSPoint):
    worldid = wp.tid()
    lo.alpha[worldid] = p0.alpha[worldid] - p0.deriv_0[worldid] / p0.deriv_1[worldid]
  wp.launch(_lo_alpha, dim=(d.nworld), inputs=[ls_ctx.lo, ls_ctx.p0])
  ...
  def _tree_map(ls_ctx: LSContext):
    ...
    ls_ctx.hi.alpha[worldid] = lesser * ls_ctx.p0.alpha[worldid] + not_lesser * ls_ctx.lo.alpha[worldid]
    ls_ctx.hi.cost[worldid] = lesser * ls_ctx.p0.cost[worldid] + not_lesser * ls_ctx.lo.cost[worldid]
    ls_ctx.hi.deriv_0[worldid] = lesser * ls_ctx.p0.deriv_0[worldid] + not_lesser * ls_ctx.lo.deriv_0[worldid]
    ls_ctx.hi.deriv_1[worldid] = lesser * ls_ctx.p0.deriv_1[worldid] + not_lesser * ls_ctx.lo.deriv_1[worldid]

    ls_ctx.lo.alpha[worldid] = lesser * ls_ctx.lo.alpha[worldid] + not_lesser * ls_ctx.p0.alpha[worldid]
    ls_ctx.lo.cost[worldid] = lesser * ls_ctx.lo.cost[worldid] + not_lesser * ls_ctx.p0.cost[worldid]
    ls_ctx.lo.deriv_0[worldid] = lesser * ls_ctx.lo.deriv_0[worldid] + not_lesser * ls_ctx.p0.deriv_0[worldid]
    ls_ctx.lo.deriv_1[worldid] = lesser * ls_ctx.lo.deriv_1[worldid] + not_lesser * ls_ctx.p0.deriv_1[worldid]
  wp.launch(_tree_map, dim=(d.nworld), inputs=[ls_ctx])

  ls_ctx.swap.fill_(1)
  ls_ctx.ls_iter.fill_(0)

  for i in range(wp.static(m.opt.ls_iterations)):
    static_i = wp.static(i)

    @wp.kernel
    def _alpha_next(ctx: Context, ls_pnt: LSPoint):
      worldid = wp.tid()
      ctx.alpha[worldid] = ls_pnt.alpha[worldid] - ls_pnt.deriv_0[worldid] / ls_pnt.deriv_1[worldid]
    ...
    def _swap_lo_hi(ls_ctx: LSContext):
      ...
      lo_alpha = float(swap_lo_next) * lo_next_alpha + (1.0 - float(swap_lo_next)) * lo_alpha
      lo_cost = float(swap_lo_next) * lo_next_cost + (1.0 - float(swap_lo_next)) * lo_cost
      lo_deriv_0 = float(swap_lo_next) * lo_next_deriv_0 + (1.0 - float(swap_lo_next)) * lo_deriv_0
      lo_deriv_1 = float(swap_lo_next) * lo_next_deriv_1 + (1.0 - float(swap_lo_next)) * lo_deriv_1
      ...
      lo_deriv_0 = float(swap_lo_mid) * mid_deriv_0 + (1.0 - float(swap_lo_mid)) * lo_deriv_0
      lo_deriv_1 = float(swap_lo_mid) * mid_deriv_1 + (1.0 - float(swap_lo_mid)) * lo_deriv_1

      swap_lo_hi_next = _in_bracket(lo_deriv_0, hi_next_deriv_0)
      lo_alpha = float(swap_lo_hi_next) * hi_next_alpha + (1.0 - float(swap_lo_hi_next)) * lo_alpha
      lo_cost = float(swap_lo_hi_next) * hi_next_cost + (1.0 - float(swap_lo_hi_next)) * lo_cost
      lo_deriv_0 = float(swap_lo_hi_next) * hi_next_deriv_0 + (1.0 - float(swap_lo_hi_next)) * lo_deriv_0
      lo_deriv_1 = float(swap_lo_hi_next) * hi_next_deriv_1 + (1.0 - float(swap_lo_hi_next)) * lo_deriv_1

      swap_hi_next = _in_bracket(hi_deriv_0, hi_next_deriv_0)
      hi_alpha = float(swap_hi_next) * hi_next_alpha + (1.0 - float(swap_hi_next)) * hi_alpha
      hi_cost = float(swap_hi_next) * hi_next_cost + (1.0 - float(swap_hi_next)) * hi_cost
      hi_deriv_0 = float(swap_hi_next) * hi_next_deriv_0 + (1.0 - float(swap_hi_next)) * hi_deriv_0
      hi_deriv_1 = float(swap_hi_next) * hi_next_deriv_1 + (1.0 - float(swap_hi_next)) * hi_deriv_1
      ...
      hi_deriv_0 = float(swap_hi_mid) * mid_deriv_0 + (1.0 - float(swap_hi_mid)) * hi_deriv_0
      hi_deriv_1 = float(swap_hi_mid) * mid_deriv_1 + (1.0 - float(swap_hi_mid)) * hi_deriv_1

      swap_hi_lo_next = _in_bracket(hi_deriv_0, lo_next_deriv_0)
      hi_alpha = float(swap_hi_lo_next) * lo_next_alpha + (1.0 - float(swap_hi_lo_next)) * hi_alpha
      hi_cost = float(swap_hi_lo_next) * lo_next_cost + (1.0 - float(swap_hi_lo_next)) * hi_cost
      hi_deriv_0 = float(swap_hi_lo_next) * lo_next_deriv_0 + (1.0 - float(swap_hi_lo_next)) * hi_deriv_0
      hi_deriv_1 = float(swap_hi_lo_next) * lo_next_deriv_1 + (1.0 - float(swap_hi_lo_next)) * hi_deriv_1
    ...
    def _done(ls_ctx: LSContext, ctx: Context, m: types.Model, ls_iter: int):
      ...
      done = done or ~ls_ctx.swap[worldid]
      done = done or ((ls_ctx.lo.deriv_0[worldid] < 0.0) and (ls_ctx.lo.deriv_0[worldid] > -ctx.gtol[worldid]))
      done = done or ((ls_ctx.hi.deriv_0[worldid] > 0.0) and (ls_ctx.hi.deriv_0[worldid] < ctx.gtol[worldid]))
      ls_ctx.done[worldid] = int(ls_ctx.done[worldid] or done)
    wp.launch(_done, dim=(d.nworld), inputs=[ls_ctx, ctx, m, static_i])
  ...
  def _alpha(ctx: Context, ls_ctx: LSContext):
    ...
    ctx.alpha[worldid] = float(~ctx.done[worldid]) * improvement * (lo_hi_cost * ls_ctx.lo.alpha[worldid] + (1.0 - lo_hi_cost) * ls_ctx.hi.alpha[worldid])
  ...
  def _jaref(ctx: Context, d: types.Data):
    worldid, efcid = wp.tid()
    ctx.Jaref[worldid, efcid] += ctx.alpha[worldid] * ctx.jv[worldid, efcid]
  wp.launch(_jaref, dim=(d.nworld, d.nefc), inputs=[ctx, d])
...
def solve(m: types.Model, d: types.Data):
  ...
  # warmstart
  # TODO(team): warmstart disable
  qacc = d.qacc_smooth

  wp.copy(d.qacc, d.qacc_warmstart)
  _create_context(ctx, m, d, grad=False)
  wp.copy(ctx.cost_warmstart, ctx.cost)

  wp.copy(d.qacc, d.qacc_smooth)
  _create_context(ctx, m, d, grad=False)

  @wp.kernel
  def _qacc(ctx: Context, d: types.Data, qacc: wp.array(ndim=2, dtype=wp.float32)):
    worldid, dofid = wp.tid()
    warm_improve = float(ctx.cost_warmstart[worldid] < ctx.cost[worldid])
    qacc[worldid, dofid] = warm_improve * d.qacc_warmstart[worldid, dofid] + (1.0 - warm_improve) * d.qacc_smooth[worldid, dofid]
  wp.launch(_qacc, dim=(d.nworld, m.nv), inputs=[ctx, d, qacc])

  wp.copy(d.qacc, qacc)

  _create_context(ctx, m, d, grad=True)

  for i in range(wp.static(m.opt.iterations)):
    static_i = wp.static(i)
    ...
    if wp.static(m.opt.solver) == 2:  # Newton
      @wp.kernel
      ...
      def _beta_num_den(ctx: Context):
        ...
        wp.atomic_add(ctx.beta_num, worldid, ctx.grad[worldid, dofid] * (ctx.Mgrad[worldid, dofid] - prev_Mgrad))
      ...
      def _beta(ctx: Context):
        worldid = wp.tid()
        ctx.beta[worldid] = wp.max(0.0, ctx.beta_num[worldid] / wp.max(mujoco.mjMINVAL, ctx.beta_den[worldid]))
      wp.launch(_beta, dim=(d.nworld), inputs=[ctx])

      @wp.kernel
      def _search_cg(ctx: Context):
        worldid, dofid = wp.tid()
        ctx.search[worldid, dofid] = -1.0 * ctx.Mgrad[worldid, dofid] + ctx.beta[worldid] * ctx.search[worldid, dofid]
    ...
    def _done(ctx: Context, m: types.Model, solver_niter: int):
      ...
      ctx.done[worldid] = int(ctx.done[worldid] or done)
    wp.launch(_done, dim=(d.nworld), inputs=[ctx, m, static_i])

...
def _create_context(ctx: Context, m: types.Model, d: types.Data, grad: bool = True):
  ...
  def _jaref(ctx: Context, m: types.Model, d: types.Data):
    ...
    wp.atomic_add(
      ctx.Jaref[worldid],
      efcid,
      d.efc_J[worldid, efcid, dofid] * d.qacc[worldid, dofid]
      - d.efc_aref[worldid, efcid] / float(m.nv),
    )
...
def _update_constraint(m: types.Model, d: types.Data, ctx: Context):
  ...
  def _efc_force(ctx: Context, d: types.Data):
    worldid, efcid = wp.tid()
    ctx.efc_force[worldid, efcid] = (
      -1.0
      * d.efc_D[worldid, efcid]
      * ctx.Jaref[worldid, efcid]
      * float(ctx.active[worldid, efcid])
    )
  ...
  def _cost(ctx: Context, d: types.Data):
    ...
    wp.atomic_add(
      ctx.cost,
      worldid,
      0.5 * d.efc_D[worldid, efcid] * Jaref * Jaref * float(ctx.active[worldid, efcid]),
    )
  ...
  def _qfrc_constraint(ctx: Context, d: types.Data):
    ...
    wp.atomic_add(
      ...
      dofid,
      d.efc_J[worldid, efcid, dofid] * ctx.efc_force[worldid, efcid],
    )
...
def _update_gradient(m: types.Model, d: types.Data, ctx: Context):
  ...
  if m.opt.solver == 1:  # CG
    ...
    @wp.kernel
    def _JTDAJ(
      J: wp.array(ndim=3, dtype=wp.float32),
      D: wp.array(ndim=2, dtype=wp.float32),
      A: wp.array(ndim=2, dtype=wp.int32),
      output: wp.array(ndim=3, dtype=wp.float32),
    ):
      worldid, dofi, dofj = wp.tid()
      sum = float(0.0)
      for i in range(NEFC):
        sum += (
          J[worldid, i, dofi]
      ...
      output[worldid, dofi, dofj] += sum

    wp.launch(
      _JTDAJ, dim=(d.nworld, m.nv, m.nv), inputs=[d.efc_J, d.efc_D, ctx.active, ctx.h]
    )
...
def _linesearch(m: types.Model, d: types.Data, ctx: Context):
  ...
  def _jv(ctx: Context, d: types.Data):
    ...
    wp.atomic_add(
      ctx.jv[worldid],
      efcid,
      d.efc_J[worldid, efcid, dofid] * ctx.search[worldid, dofid],
    )

...
class SolverTest(parameterized.TestCase):
  ...
  def _load(self, fname: str, is_sparse: bool = True, cone: int = mujoco.mjtCone.mjCONE_PYRAMIDAL, solver_: int = mujoco.mjtSolver.mjSOL_NEWTON, iterations: int = 2):
    path = epath.resource_path('mujoco.mjx') / 'test_data' / fname
    mjm = mujoco.MjModel.from_xml_path(path.as_posix())
    mjm.opt.jacobian = is_sparse
    mjm.opt.iterations = iterations
    mjm.opt.cone = cone  # pyramidal==0
    mjm.opt.solver = solver_  # CG==1, Newton==2
    mjm.opt.disableflags |= mujoco.mjtDisableBit.mjDSBL_FRICTIONLOSS
    mjd = mujoco.MjData(mjm)
    mujoco.mj_resetDataKeyframe(mjm, mjd, 1)
  ...
  def test_solve_cg_pyramidal(self, cone, solver_, iterations):
    """Tests MJX solve."""
    mjm, mjd, m, d = self._load('humanoid/humanoid.xml', is_sparse=False, cone=cone, solver_=solver_, iterations=iterations)
    ...
    for arr in (d.qacc, d.qfrc_constraint, d.efc_force):
      arr.zero_()
